...
| `Annotated[str, ...]` constraints   | `TAG`           | Same as `str` — the constraint validates input but does not change the index type.                  |
| `str` + `Field(full_text_search=True)` | `TEXT`       | Adds a separate `<field>_fts TEXT` field alongside the `TAG` for full-text search.                   |
