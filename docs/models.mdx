...
#### Raw-byte vector fields on `HashModel`

On a `HashModel` you can instead annotate the field as `bytes` and store the
packed vector buffer directly. The blob is written to the hash as raw binary
(no JSON array, no base64), so a 1536-dim `FLOAT16` embedding occupies
`1536 * 2 = 3072` bytes exactly:
...
> **NOTE — reading raw-blob vectors back requires `decode_responses=False`.**
...
> they return scores and primary keys only, never the raw blob).
