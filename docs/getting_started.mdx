...
description: Install Redis OM, define your first model, and save and query data.
---
...
#### Docker with the `valkey/valkey:9-alpine` image

For a lean deployment without search modules (HashModel only, no rich queries):

    $ docker run -d -p 6379:6379 valkey/valkey:9-alpine

With ValkeySearch + ValkeyJSON for full feature parity:

    $ docker run -d -p 6379:6379 valkey/valkey:9-alpine --loadmodule /usr/lib/valkey/modules/valkeyjson.so --loadmodule /usr/lib/valkey/modules/valkeysearch.so
...
The current release of redis-om-python also ships several top-level helpers
that take direct advantage of newer Redis capabilities. Each one is documented
and tested independently:

### Redis 8.8+

- **[AtomicCounter](/atomic_counter)** — atomic increment with bounds,
  saturation, expiration, and `ENX`. Drop-in for rate limiters and capped
  counters.
- **[Redis Arrays](/redis_arrays)** — sparse, index-addressable arrays with
  ring buffers, `AROP` aggregates, and `ARGREP` text search.
- **[Atomic Strings](/atomic_strings)** — `AtomicString` (`SET IFEQ` / `IFNE`,
  `DELEX`, `DIGEST`) and bulk `MSETEX` for compare-and-set style writes.
- **[Vector Sets](/vector_sets)** — `VectorSet` wrapper over the HNSW-backed
  `@vectorset` module for approximate nearest-neighbour queries.
- **[Sorted Set Aggregations](/sorted_set_aggregations)** — `ZUNION` / `ZINTER`
  with `AGGREGATE COUNT` for co-occurrence counting.

### Redis 8.6+

- **[Hot Keys Tracker](/hotkeys)** — `HOTKEYS START/GET/STOP/RESET` server-side
  top-K sampling for CPU and network-byte attribution.

### Redis 8.2+

- **[Bitmap Operations](/bitmap_ops)** — `BITOP DIFF` / `DIFF1` / `ANDOR` /
  `ONE` via `BitmapOps`.
- **[Cluster Admin](/cluster_admin)** — `CLUSTER SLOT-STATS` and
  `CLUSTER MIGRATION` introspection for Redis 8.2+ clusters.

### Any Redis version

- **[Keyspace Notifications](/keyspace_notifications)** — constants and helpers
  for `notify-keyspace-events` (Redis 2.8+).
- **[OpenTelemetry Observability](/observability)** — opt-in metrics via
  redis-py 8.0 instrumentation.

## Reference

- [Queries and Vector Search](/queries) — full expression syntax, KNN, pagination, and cursors.
- [Bulk Operations](/bulk_operations) — `add()`, `get_many()`, `delete_many()`.
- [Migrations](/migrations) — schema management and the `migrate` CLI.

## Next Steps

Now that you know the basics of working with Redis OM, start playing around with it in your project!

If you're a FastAPI user, check out [how to integrate Redis OM with FastAPI](/fastapi_integration).
