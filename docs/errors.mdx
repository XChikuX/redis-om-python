...
description: Common errors raised by Redis OM and how to fix them.
---
...
# Will work now that `first_name` is indexed
Member.find(Member.first_name == "Andrew").all()
```
...
> You passed a sequence value (...) to operator EQ. Only IN (<<) and NOT_IN (>>) support sequence values.

The scalar comparison operators (`==`, `!=`, `<`, `<=`, `>`, `>=`,
`LIKE` (`%`), `STARTSWITH`, `ENDSWITH`, `CONTAINS`) expect a single value on
the right-hand side. Lists, tuples, and other `collections.abc.Sequence`
containers are only meaningful with `IN` (`<<`) and `NOT_IN` (`>>`).

If you wanted to check that a field equals **any of** several values, use
`IN`:
...
# ✗ This raises QueryNotSupportedError (E9):
# Member.find(Member.last_name == ["Brookins", "Smith"])

# ✓ Use IN (<<) instead:
results = Member.find(
    Member.last_name << ["Brookins", "Smith"]
).all()
```

If you only meant to compare against a single value, drop the brackets:

```python
results = Member.find(Member.last_name == "Brookins").all()
```

The validator does not reject `str` or `bytes` on the right-hand side of a
scalar operator, because both are deliberately excluded from
`collections.abc.Sequence` in this check (see
[tests/test_find_query.py](https://github.com/XChikuX/redis-om-python/blob/main/tests/test_find_query.py)
for the exact boundary, which also covers `range`, `collections.deque`,
empty lists, single-element lists, and tuples).

## Redis exceptions

You never need to import `redis` directly to catch Redis failures — Redis OM
re-exports redis-py's exception types. The full set lives in
`redis_om.exceptions`, and the most common ones are available from the package
root:
...
`redis_om.exceptions` mirrors `redis.exceptions`: every redis-py exception is
available under its redis-py name. The two exceptions that shadow Python
builtins are exposed as `RedisConnectionError` and `RedisTimeoutError` instead
of `ConnectionError` and `TimeoutError`.

The async API exposes the same module as `aredis_om.exceptions`.
